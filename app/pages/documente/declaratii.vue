  ...
  <div class="space-y-6">
    <div class="bg-white rounded-lg shadow p-6">
      <div class="flex justify-between items-center mb-6">
        <h2 class="text-xl md:text-2xl font-bold text-gray-900">Declarații</h2>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        >
          <UploadDocument tip="declaratie" @success="handleUploadSuccess" />
        </div>
        <div class="md:col-span-2 md:order-1">
          <DocumentsList tip="declaratie" :year="selectedYear" />
        </div>
      </div>
    </div>
...
const handleUploadSuccess = () => {
  documentsStore.fetchDocuments("declaratie", true);
};

onMounted(async () => {
  try {
    await documentsStore.fetchDocuments("declaratie");
  } catch (error) {
