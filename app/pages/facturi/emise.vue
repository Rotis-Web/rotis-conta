  ...
  <div class="space-y-6">
    <div class="bg-white rounded-lg shadow p-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        >
          <UploadDocument tip="factura-emisa" @success="handleUploadSuccess" />
        </div>
        <div class="md:col-span-2 md:order-1">
          <DocumentsList tip="factura-emisa" :year="selectedYear" />
        </div>
      </div>
    </div>
...
const handleUploadSuccess = () => {
  documentsStore.fetchDocuments("factura-emisa", true);
};

onMounted(async () => {
  try {
    await documentsStore.fetchDocuments("factura-emisa");
  } catch (error) {
